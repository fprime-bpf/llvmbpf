...
*/
Expected<ThreadSafeModule> llvm_bpf_jit_context::generateModuleWithSS1(
	...
	uintptr_t register_state_store_addr, uint16_t fixed_heap_size)
{
	...
	if (per_instruction_state) {
		// LDDW's second slot is data, not an independently executable
		// instruction. Every other instruction is a resume target.
		for (uint16_t pc = 0; pc < insts.size(); ++pc) {
			if (pc > 0 && insts[pc - 1].opcode == EBPF_OP_LDDW)
				continue;
	...
	Value *invalidResumePc = nullptr;
	{
		...
		// Create registers
		if (per_instruction_state) {
			auto *state = reinterpret_cast<const ExecState *>(
				register_state_store_addr);
			auto pointerConstant = [&](const void *ptr) -> Constant * {
				return ConstantExpr::getIntToPtr(
					builder.getInt64(reinterpret_cast<uintptr_t>(ptr)),
					builder.getPtrTy());
			};
			auto *stateBase = pointerConstant(state);
			inputSnapshot = bpf_func->getArg(1);
			hasInputSnapshot = builder.CreateICmpNE(
				inputSnapshot,
				ConstantPointerNull::get(builder.getPtrTy()));
			safeInputSnapshot = builder.CreateSelect(
				hasInputSnapshot, inputSnapshot, stateBase,
				"safeInputSnapshot");
			...
			// Restore the caller-provided snapshot into the fixed state
			// storage. A zero length makes the null-snapshot path a no-op.
			auto *restoreStateBytes = builder.CreateSelect(
				hasInputSnapshot,
				builder.getInt64(offsetof(ExecState, heap)),
				builder.getInt64(0));
			builder.CreateMemCpy(stateBase, MaybeAlign(alignof(uint64_t)),
					 safeInputSnapshot,
					 MaybeAlign(alignof(uint64_t)),
					 restoreStateBytes);
			auto *sourceFpu = inputField(offsetof(ExecState, fpuRegs),
						     builder.getFloatTy());
			auto *destFpu = execStateField(offsetof(ExecState, fpuRegs),
						   builder.getFloatTy());
			auto *restoreFpuBytes = builder.CreateSelect(
				hasInputSnapshot,
				builder.getInt64(sizeof(ExecState::fpuRegs)),
				builder.getInt64(0));
			builder.CreateMemCpy(destFpu, MaybeAlign(alignof(float)),
					 sourceFpu, MaybeAlign(alignof(float)),
					 restoreFpuBytes);

			auto *dataOffset = builder.CreateSelect(
				hasInputSnapshot,
				builder.CreateLoad(builder.getInt32Ty(), inputField(
					offsetof(ExecState, dataStackOffset),
					builder.getInt32Ty())),
				builder.getInt32(frameSize));
			auto *callCount = builder.CreateSelect(
				hasInputSnapshot,
				builder.CreateLoad(builder.getInt16Ty(), inputField(
					offsetof(ExecState, callStackSize),
					builder.getInt16Ty())),
				builder.getInt16(0));
			builder.CreateStore(dataOffset, dataStackOffsetSlot);
			builder.CreateStore(callCount, callStackSizeSlot);
			auto *resumePc = builder.CreateSelect(
				hasInputSnapshot,
				builder.CreateLoad(builder.getInt16Ty(), inputField(
					offsetof(ExecState, pc), builder.getInt16Ty())),
				builder.getInt16(0));
			builder.CreateStore(resumePc, pcSnapshotSlot);

			Value *stackBegin = pointerConstant(state->dataStack);
			stackEnd = builder.CreateGEP(
				builder.getInt8Ty(), stackBegin,
				{ builder.getInt32(dataStackSize) }, "stackEnd");
			auto *copyDataBytes = builder.CreateSelect(
				hasInputSnapshot,
				builder.CreateZExt(dataOffset, builder.getInt64Ty()),
				builder.getInt64(0));
			auto *sourceStack = builder.CreateLoad(
				builder.getPtrTy(), inputField(offsetof(ExecState, dataStack),
							 builder.getPtrTy()));
			auto *sourceLiveData = builder.CreateGEP(
				builder.getInt8Ty(), sourceStack,
				{ builder.CreateSub(builder.getInt64(dataStackSize),
						    copyDataBytes) });
			auto *destLiveData = builder.CreateGEP(
				builder.getInt8Ty(), stackBegin,
				{ builder.CreateSub(builder.getInt64(dataStackSize),
						    copyDataBytes) });
			builder.CreateMemCpy(destLiveData, MaybeAlign(1), sourceLiveData,
					 MaybeAlign(1), copyDataBytes);

			callStack = pointerConstant(state->callStack);
			auto *copyCallBytes = builder.CreateSelect(
				hasInputSnapshot,
				builder.CreateMul(
					builder.CreateZExt(callCount, builder.getInt64Ty()),
					builder.getInt64(sizeof(void *))),
				builder.getInt64(0));
			auto *sourceCallStack = builder.CreateLoad(
				builder.getPtrTy(), inputField(offsetof(ExecState, callStack),
							 builder.getPtrTy()));
			builder.CreateMemCpy(callStack, MaybeAlign(alignof(void *)),
					 sourceCallStack, MaybeAlign(alignof(void *)),
					 copyCallBytes);

			if (fixed_heap_size != 0) {
				auto *sourceHeap = builder.CreateLoad(
					builder.getPtrTy(), inputField(offsetof(ExecState, heap),
								 builder.getPtrTy()));
				auto *copyHeapBytes = builder.CreateSelect(
					hasInputSnapshot,
					builder.getInt64(fixed_heap_size),
					builder.getInt64(0));
				builder.CreateMemCpy(pointerConstant(state->heap),
						 MaybeAlign(1), sourceHeap, MaybeAlign(1),
						 copyHeapBytes);
			}
			...
			for (int i = 0; i <= 10; ++i)
				fregs.push_back(execStateField(
					offsetof(ExecState, fpuRegs) + i * sizeof(float),
					builder.getFloatTy()));
			callItemCnt = callStackSizeSlot;

			// A fresh invocation starts at the top frame. A resume keeps
			// all restored registers, including r1/r2.
			auto *heapAddress = builder.CreatePtrToInt(
				pointerConstant(state->heap), builder.getInt64Ty());
			...
			builder.CreateStore(builder.CreateSelect(
				...
				builder.getInt64(fixed_heap_size)), regs[2]);
			memSnapshotSrc = pointerConstant(state->heap);
			memSnapshotLen = builder.getInt64(fixed_heap_size);
		} else {
		...
		if (main_func_with_arguments) {
			...
			builder.CreateStore(inputSnapshot
		...
		BasicBlock::Create(*context, "localFuncReturnBlock", bpf_func);
	{
		...
		builder.CreateStore(builder.CreateSub(count,
				    ...
				    callItemCnt);
		if (per_instruction_state) {
			auto *offset = builder.CreateLoad(builder.getInt32Ty(),
						      dataStackOffsetSlot);
			builder.CreateStore(
